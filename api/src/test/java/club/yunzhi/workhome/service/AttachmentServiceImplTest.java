...
class AttachmentServiceImplTest extends ServiceTest{
    ...
    void uploadWork() throws IOException {
        ...
        Mockito.doReturn(attachment).when(attachmentService).uploadWork(multipartFile);

        Assertions.assertEquals(attachment, this.attachmentService.uploadWork(multipartFile));
    }
}